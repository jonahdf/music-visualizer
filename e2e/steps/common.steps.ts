...
Given('the menu is open on the {word} tab', async ({ appPage }, tab: string) => {
  await openMenu(appPage);
  await goToTab(appPage, tab as 'Presets' | 'Audio' | 'Settings' | 'Create');
});
...
Then('the active preset should have changed', async ({ appPage }) => {
  ...
  const after = (await appPage.locator('.bar-preset-name').textContent())?.trim() ?? '';
...
When('I upload the test audio file', async ({ appPage }) => {
  // Navigate to Audio tab (open menu if needed) then upload.
...
Given('auto-advance is set to {string}', async ({ appPage }, label: string) => {
  // Navigate to Settings tab (open menu if needed) and set the interval.
  // Does NOT close the menu — callers that need the menu closed should do so explicitly.
  await openMenu(appPage);
  await goToTab(appPage, 'Settings');
...
Given('auto-advance is held', async ({ appPage }) => {
  // The hold button lives in the Settings tab (.hold-btn) when the menu is open.
...
Given('audio is muted', async ({ appPage }) => {
  ...
  const isMuted = await muteBtn.evaluate((el) => el.classList.contains('bar-active'));
  if (!isMuted) {
